...
const bool kDefaultUnwrappingFlag(false);

void PrintUsage(std::ostream* stream) {
  // clang-format off
  *stream << std::endl;
  *stream << " phase - transform real sequence to phase" << std::endl;
  *stream << std::endl;
  ...
  *stream << "       -u    : unwrapping                        (  bool)[" << std::setw(5) << std::right << sptk::ConvertBooleanToString(kDefaultUnwrappingFlag) << "]" << std::endl;  // NOLINT
  *stream << "       -h    : print this message" << std::endl;
  *stream << "  infile:" << std::endl;
  *stream << "       data sequence                             (double)[stdin]" << std::endl;  // NOLINT
  *stream << "  stdout:" << std::endl;
  *stream << "       phase [pi rad]                            (double)" << std::endl;  // NOLINT
 ...
 *   - double-type phase
 *
 * The below example calculates phase spectrum from 10-th order filter
 * coefficients:
 *
 * @code{.sh}
